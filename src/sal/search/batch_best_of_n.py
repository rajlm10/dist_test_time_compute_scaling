...
def best_of_n(batch_x, config: Config, model, tokenizer, prm: PRM):
    """
    Distributed best-of-n approach for batched inputs.
    Each GPU (process) generates one candidate completion per step for each query in the batch.
    Rank 0 gathers all candidate completions, ranks them using PRM,
    picks the best candidate for each query, and then broadcasts the partial answers for the next step.
    """
    ...
    for x in batch_x:
        ...
        conv = tokenizer.apply_chat_template(
            [init_conv], tokenize=False, add_generation_prompt=True
        )[0]
