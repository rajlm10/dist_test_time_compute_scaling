...
def best_of_n(x, config: Config, llm: LLM, prm: PRM):
    ...
    for i in range(len(completions)):
        steps.append(
            max([len(re.findall(pattern_steps, output.text, flags=re.DOTALL))
    ...
    for i in range(len(completions)):
        completions[i] = [
            re.findall(pattern_steps, output.text, flags=re.DOTALL)[0].strip()
            for r in responses[i * config.n : (i + 1) * config.n]
            for output in r.outputs
        ]
    ...
    pred = [completion[np.argmax(s)] for completion, s in zip(completions, agg_scores)][0]
    ...
    for i in range(1,steps[0]):
        ...
        for j in range(len(completions)):
            completions[j] = [
                format_with_steps(re.findall(pattern_steps, output.text, flags=re.DOTALL)[:i+1])
                for r in responses[j * config.n : (j + 1) * config.n]
                for output in r.outputs
            ]
